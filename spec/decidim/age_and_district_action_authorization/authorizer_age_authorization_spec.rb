...
RSpec.describe Decidim::AgeAndDistrictActionAuthorization::Authorizer do
  ...
  it "authorizes with age metadata" do
    authorizer = authorizer_class.new(authorization_for("1970/11/21"), { "min_age" => "20" }, component, resource)
    expect(authorizer.authorize).to include(:ok)
  end

  it "uses 18 as default age when not present" do
    old = 18.years.ago.strftime("%Y/%m/%d")
    auth_old = authorizer_class.new(authorization_for(old), { "min_age" => nil }, component, resource)
    expect(auth_old.authorize).to include(:ok)

    young = 17.years.ago.strftime("%Y/%m/%d")
    auth_young = authorizer_class.new(authorization_for(young), { "min_age" => nil }, component, resource)
  ...
  it "does not authorize the user if the authorization is not present" do
    authorizer_without_authorization = authorizer_class.new(nil, { "min_age" => nil }, component, resource)
  ...
  describe "when max_age is pressent" do
    it "authorizes if is older that :age and yonger that :max_age" do
      ...
      authorizer = authorizer_class.new(authorization_for(birthdate), { "min_age" => "20", "max_age" => "22" }, component, resource)
      expect(authorizer.authorize).to include(:ok)
    end
